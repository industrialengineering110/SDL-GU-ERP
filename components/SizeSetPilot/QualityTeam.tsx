...
import { CheckCircle, Play, CheckCircle2, Search, AlertTriangle, Plus, X } from 'lucide-react';
...
const QualityTeam: React.FC<Props> = ({ requests, onUpdate }) => {
  const qualityRequests = requests.filter(r => r.currentStage === 'Quality Team');
  ...
  const handleEnd = (id: string) => {
    ...
    updatedStages['Wash Sample Concern'] = {
      ...updatedStages['Wash Sample Concern'],
      quantityHandled: updatedStages['Quality Team'].quantityHandled,
      status: 'Pending'
    };
    onUpdate(id, { 
      stages: updatedStages,
      currentStage: 'Wash Sample Concern'
    });
  };
  ...
  const addRiskPoint = (reqId: string, point: string) => {
    ...
    if (!req) return;
    const updatedStages = { ...req.stages };
    updatedStages['Quality Team'].riskPoints = [...(updatedStages['Quality Team'].riskPoints || []), point];
  ...
  const removeRiskPoint = (reqId: string, index: number) => {
    ...
    if (!req) return;
    const updatedStages = { ...req.stages };
    updatedStages['Quality Team'].riskPoints = updatedStages['Quality Team'].riskPoints.filter((_, i) => i !== index);
    onUpdate(reqId, { stages: updatedStages });
  };
    ...
    <div className="bg-card p-8 rounded-[2.5rem] border border-border shadow-2xl shadow-black/5">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight">Quality Team</h2>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Inspection & Approval</p>
          </div>
        </div>
      </div>

      <div className="space-y-6">
        {qualityRequests.map(req => (
          <div key={req.id} className="bg-muted/30 p-6 rounded-3xl border border-border group hover:border-primary/30 transition-all">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-muted-foreground">Sewing Qty</p>
                <p className="text-sm font-black">{req.stages['Sewing / Sample Line Concern'].quantityHandled}</p>
              </div>
              <div className="flex items-center justify-end">
                {req.stages['Quality Team'].status === 'Pending' && (
                  <button onClick={() => handleStart(req.id)} className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:translate-y-[-2px] transition-all shadow-lg shadow-blue-600/20">
                    <Play size={14} /> Start Quality
                  </button>
                )}
                {req.stages['Quality Team'].status === 'In Progress' && (
                  <button onClick={() => handleEnd(req.id)} className="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:translate-y-[-2px] transition-all shadow-lg shadow-emerald-600/20">
                    <CheckCircle2 size={14} /> Complete Quality
                  </button>
                )}
                {req.stages['Quality Team'].status === 'Completed' && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-muted-foreground">Passed Qty</label>
                <input 
                  type="number" 
                  value={req.stages['Quality Team'].quantityHandled || ''} 
                  onChange={e => {
                    const updatedStages = { ...req.stages };
                    updatedStages['Quality Team'].quantityHandled = parseInt(e.target.value) || 0;
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-muted-foreground">Rejects</label>
                <input 
                  type="number" 
                  value={req.stages['Quality Team'].rejectionQty || ''} 
                  onChange={e => {
                    const updatedStages = { ...req.stages };
                    updatedStages['Quality Team'].rejectionQty = parseInt(e.target.value) || 0;
                    onUpdate(req.id, { stages: updatedStages });
                  }}
                  className="w-full p-4 bg-rose-50 border-none rounded-2xl text-sm font-black text-rose-600 focus:ring-2 focus:ring-rose-500/20 outline-none"
                />
              </div>
              <div className="space-y-2">
                ...
                <input 
                  type="text" 
                  placeholder="Remarks..."
                  value={req.stages['Quality Team'].remarks} 
                  onChange={e => {
                    const updatedStages = { ...req.stages };
                    updatedStages['Quality Team'].remarks = e.target.value;
                    onUpdate(req.id, { stages: updatedStages });
                  }}
                  className="w-full p-4 bg-white border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-primary/20 outline-none"
                />
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {req.stages['Quality Team'].riskPoints?.map((point, index) => (
        ...
        {qualityRequests.length === 0 && (
          <div className="p-12 text-center bg-card rounded-3xl border border-border">
            <div className="flex flex-col items-center gap-2 opacity-20">
              <Search size={48} />
              <span className="text-xs font-black uppercase tracking-widest">No pending quality checks</span>
            </div>
          </div>
        )}
      </div>

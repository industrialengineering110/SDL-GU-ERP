...
import { Droplets, Play, CheckCircle2, Search, AlertTriangle, Plus, X } from 'lucide-react';
...
const WashConcern: React.FC<Props> = ({ requests, onUpdate }) => {
  const washRequests = requests.filter(r => r.currentStage === 'Wash Sample Concern');
  ...
  const addRiskPoint = (reqId: string, point: string) => {
    ...
    if (!req) return;
    const updatedStages = { ...req.stages };
    updatedStages['Wash Sample Concern'].riskPoints = [...(updatedStages['Wash Sample Concern'].riskPoints || []), point];
  ...
  const removeRiskPoint = (reqId: string, index: number) => {
    ...
    if (!req) return;
    const updatedStages = { ...req.stages };
    updatedStages['Wash Sample Concern'].riskPoints = updatedStages['Wash Sample Concern'].riskPoints.filter((_, i) => i !== index);
    onUpdate(reqId, { stages: updatedStages });
  };
    ...
    <div className="bg-card p-8 rounded-[2.5rem] border border-border shadow-2xl shadow-black/5">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="p-4 bg-blue-400 text-white rounded-2xl shadow-lg shadow-blue-400/20">
            <Droplets size={24} />
          </div>
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight">Wash Concern</h2>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Sample Washing & Finishing</p>
          </div>
        </div>
      </div>

      <div className="space-y-6">
        {washRequests.map(req => (
          <div key={req.id} className="bg-muted/30 p-6 rounded-3xl border border-border group hover:border-primary/30 transition-all">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-muted-foreground">Quality Qty</p>
                <p className="text-sm font-black">{req.stages['Quality Team'].quantityHandled}</p>
              </div>
              <div className="flex items-center justify-end">
                {req.stages['Wash Sample Concern'].status === 'Pending' && (
                ...
                {req.stages['Wash Sample Concern'].status === 'In Progress' && (
                ...
                {req.stages['Wash Sample Concern'].status === 'Completed' && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-muted-foreground">Wash Qty</label>
                <input 
                  type="number" 
                  value={req.stages['Wash Sample Concern'].quantityHandled || ''} 
                  onChange={e => {
                    const updatedStages = { ...req.stages };
                    updatedStages['Wash Sample Concern'].quantityHandled = parseInt(e.target.value) || 0;
              ...
              <div className="space-y-2">
                ...
                <input 
                  type="text" 
                  placeholder="Remarks..."
                  value={req.stages['Wash Sample Concern'].remarks} 
                  onChange={e => {
                    const updatedStages = { ...req.stages };
                    updatedStages['Wash Sample Concern'].remarks = e.target.value;
                    onUpdate(req.id, { stages: updatedStages });
                  }}
                  className="w-full p-4 bg-white border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-primary/20 outline-none"
                />
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {req.stages['Wash Sample Concern'].riskPoints?.map((point, index) => (
        ...
        {washRequests.length === 0 && (
          <div className="p-12 text-center bg-card rounded-3xl border border-border">
            <div className="flex flex-col items-center gap-2 opacity-20">
              <Search size={48} />
              <span className="text-xs font-black uppercase tracking-widest">No pending wash requests</span>
            </div>
          </div>
        )}
      </div>

...
const AdminRegistration: React.FC<AdminRegistrationProps> = ({ onSuccess, embedded }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newAdmin: UserType = {
      ...
      pagePermissions: FULL_PERMISSIONS
    };

    console.log("DEBUG: Registering new admin:", JSON.stringify(newAdmin, null, 2));
    mockDb.registerUser(newAdmin);
  ...
  const content = (
    <div className={`${embedded ? 'w-full' : 'max-w-md w-full bg-card rounded-[3rem] shadow-2xl border border-border p-10 space-y-8'}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button type="submit" className="w-full bg-primary text-primary-foreground font-black py-4 rounded-2xl shadow-xl hover:bg-primary/90 transition-all flex items-center justify-center gap-2">
           <ShieldCheck size={20} /> Register Admin
        </button>
      </form>
    </div>
  );
